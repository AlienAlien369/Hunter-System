...
const STAT_META: Record<
  string,
  { icon: string; color: string; barFrom: string; barTo: string }
> = {
  str: {
    icon: "💪",
    color: "text-red-400",
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p
          className={`text-xs font-display font-bold ${unlocked ? threshold.color : "text-gray-600"}`}
        >
          {threshold.label}
        </p>
      ...
      <span
        className={`text-[10px] font-mono ${unlocked ? "text-gray-400" : "text-gray-700"}`}
      >
        {threshold.min}
      </span>
    </div>
  ...
  return (
    ...
    >
      ...
      <div className="px-5 py-4 border-b border-white/5">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-display text-white font-bold text-sm tracking-wider">
                {statKey.toUpperCase()}
              </h3>
      ...
      <div className="p-4 space-y-2">
        <p className="text-[10px] font-mono text-gray-600 tracking-wider mb-2">
          PASSIVE BUFFS
        </p>
        {tiers.map((t) => (
          <TierNode
...
export default function SkillTree() {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            <span className="ml-2 text-lg font-display font-bold">
              {available}
            </span>
          </motion.div>
          {/* Level badge */}
          <div className="px-4 py-2 rounded-xl border bg-gray-800/50 border-gray-700/30 text-gray-400 font-mono text-sm">
            ...
            <span className="ml-2 text-lg font-display font-bold text-gold">
              {profile.level}
            </span>
          </div>
        </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
            {buffs.buffs.map(({ stat, threshold }) => (
              <div
                key={`${stat}-${threshold.min}`}
                className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-800/40 border border-gray-700/30"
              >
                <span className="text-lg">{threshold.icon}</span>
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-xs font-display font-bold ${threshold.color}`}
                  >
                    {threshold.label}
                  </p>
                  <p className="text-[10px] font-mono text-gray-500 truncate">
          ...
          {buffs.penaltyReduction > 0 && (
            <p className="text-[10px] font-mono text-gray-500">
              ...
              <span className="text-green-400">
                -{Math.round(buffs.penaltyReduction * 100)}%
              </span>
            </p>
          )}
      ...
      >
        Gain {2} stat points per level up • Stats sync to the server
        automatically
      </motion.div>
    </div>
  );
