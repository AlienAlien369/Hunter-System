...
import { getStreak, getLongestStreak, getTotalActiveDays, getDayIntensity, MAX_DAILY_COMPLETIONS } from '../utils/xp';
...
function getIntensityClass(count: number): string {
  ...
  if (ratio <= 0.8) return 'bg-gradient-to-br from-orange-500 to-red-500 border-orange-400/50';
    ...
    >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-2xl font-display font-bold text-orange-400">{streak}</p>
                <p className="text-[10px] text-gray-500 font-mono tracking-wider">STREAK</p>
              </div>
            </div>
            ...
            <div className="text-center">
              <p className="text-lg font-display font-bold text-yellow-400">{longestStreak}</p>
              <p className="text-[10px] text-gray-500 font-mono tracking-wider">BEST</p>
            </div>
            ...
            {/* Total days */}
            <div className="text-center">
              <p className="text-lg font-display font-bold text-purple-400">{totalActiveDays}</p>
              <p className="text-[10px] text-gray-500 font-mono tracking-wider">TOTAL DAYS</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <div className="text-right">
              ...
              <p className="text-lg font-display font-bold text-cyan-400">{freezeCount}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 pb-4 pt-2">
        ...
        <div className="grid grid-cols-7 gap-1.5 mb-1">
          {WEEKDAYS.map(d => (
            <div key={d} className="text-center text-[10px] font-mono text-gray-500 py-1">{d}</div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7 gap-1.5">
          {cells.map((day, idx) => {
            ...
            return (
              <motion.button
                key={dateStr}
                whileHover={!isFuture && freezeCount > 0 && !isFrozen && intensity === 0 ? { scale: 1.1 } : {}}
                whileTap={!isFuture ? { scale: 0.95 } : {}}
                onClick={() => {
                  if (!isFuture && intensity === 0 && !isFrozen) handleFreezeClick(dateStr);
                }}
                disabled={isFuture}
                className={`
              ...
              >
                <span className={`text-xs font-mono ${isToday ? 'text-purple-300 font-bold' : isFrozen ? 'text-cyan-300' : intensity > 0 ? 'text-white font-bold' : 'text-gray-600'}`}>
                  {day}
                </span>
                {isFrozen && (
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-600 font-mono">Preserve a missed day</p>
            </div>
          </div>
          <button
            onClick={() => {
            ...
            >
              <div className="bg-[#0d1117] border border-cyan-500/30 rounded-xl p-6 max-w-sm w-full shadow-2xl shadow-cyan-500/10">
                <div className="text-center">
                  ...
                  <h3 className="text-lg font-display text-white font-bold mb-2">ACTIVATE STREAK FREEZE?</h3>
                  <p className="text-sm text-gray-400 mb-1">
                    Freeze <span className="text-cyan-400 font-mono">{freezeTarget}</span> to preserve your streak.
                  </p>
