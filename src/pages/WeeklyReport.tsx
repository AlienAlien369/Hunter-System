...
function computeGrade(
  ...
  if (total >= 0.9)
    return {
      ...
      message:
        "The System is impressed. Legendary performance this week, Hunter.",
    };
  if (total >= 0.75)
    return {
      grade: "A",
      label: "A-Rank Output",
      color: "text-gold",
      bg: "from-yellow-600/15 to-yellow-500/15",
      message: "Outstanding discipline. You are among the elite hunters.",
    };
  if (total >= 0.55)
    return {
      grade: "B",
      ...
      message: "Solid performance. The System acknowledges your effort.",
    };
  if (total >= 0.35)
    return {
      grade: "C",
      ...
      message: "Room for improvement. Push harder next week, Hunter.",
    };
  if (total >= 0.15)
    return {
      grade: "D",
      ...
      message: "Below expectations. The System expects more from you.",
    };
  return {
    grade: "E",
    label: "E-Rank Output",
    ...
    message: "Critical: minimal activity detected. Re-engage immediately.",
  };
}
...
export default function WeeklyReport() {
  const { profile, dailyQuests, freezeDates, loadDashboard } = useGameStore();
  ...
  const thisWeekTotalQuests = thisWeekStats.reduce(
    (s, d) => s + d.questsCompleted,
    0,
  );
  const lastWeekTotalQuests = lastWeekStats.reduce(
    (s, d) => s + d.questsCompleted,
    0,
  );
  const thisWeekActiveDays = thisWeekStats.filter(
    (d) => d.questsCompleted > 0,
  ).length;
  const lastWeekActiveDays = lastWeekStats.filter(
    (d) => d.questsCompleted > 0,
  ).length;

  const bestDay = [...thisWeekStats].sort((a, b) => b.xpEarned - a.xpEarned)[0];
  const worstDay = [...thisWeekStats]
    .filter((d) => d.questsCompleted > 0)
    .sort((a, b) => a.xpEarned - b.xpEarned)[0];
  ...
  function generateTextReport(): string {
    const lines = [
      ...
      `📅 DAY BREAKDOWN`,
      ...thisWeekStats.map(
        (d) => `  ${d.dayName}: ${d.questsCompleted} quests · ${d.xpEarned} XP`,
      ),
      ``,
      bestDay
        ? `  🏆 Best Day: ${bestDay.dayName} (${bestDay.xpEarned} XP)`
        : "",
      worstDay
        ? `  📉 Worst Day: ${worstDay.dayName} (${worstDay.xpEarned} XP)`
        : "",
      ``,
      `🔄 VS LAST WEEK`,
      ...
      <div ref={reportRef}>
        ...
        >
          <p className="text-[10px] font-mono text-gray-500 tracking-[0.3em] mb-2">
            THE SYSTEM GRADES YOUR PERFORMANCE
          </p>
          <motion.div
          ...
          >
            <span className={`text-6xl font-display font-bold ${grade.color}`}>
              {grade.grade}
            </span>
          </motion.div>
          <p className={`text-lg font-display font-bold ${grade.color}`}>
            {grade.label}
          </p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            {
              label: "XP EARNED",
              value: thisWeekTotalXp.toLocaleString(),
              icon: "⚡",
              color: "text-yellow-400",
              sub:
                xpDelta !== 0
                  ? `${xpDelta >= 0 ? "+" : ""}${xpDelta} vs last week`
                  : undefined,
            },
            {
              label: "QUESTS DONE",
              value: thisWeekTotalQuests,
              icon: "✅",
              color: "text-green-400",
              sub:
                questDelta !== 0
                  ? `${questDelta >= 0 ? "+" : ""}${questDelta} vs last week`
                  : undefined,
            },
            {
              label: "ACTIVE DAYS",
              value: `${thisWeekActiveDays}/7`,
              icon: "📅",
              color: "text-blue-400",
              sub:
                dayDelta !== 0
                  ? `${dayDelta >= 0 ? "+" : ""}${dayDelta} vs last week`
                  : undefined,
            },
            {
              label: "STREAK",
            ...
            >
              ...
              <p
                className={`text-2xl font-display font-bold ${stat.color} mt-1`}
              >
                {stat.value}
              </p>
          ...
          <div className="space-y-2">
            {thisWeekStats.map((day, i) => {
              const maxXP = Math.max(
                ...thisWeekStats.map((d) => d.xpEarned),
                1,
              );
              ...
              return (
                ...
                >
                  <span
                    className={`w-10 text-xs font-mono ${isToday ? "text-purple-400 font-bold" : "text-gray-500"}`}
                  >
                    {day.dayName}
                  </span>
          ...
          <div className="flex gap-4 mt-4 pt-4 border-t border-white/5">
            {bestDay && (
              <div className="flex-1 text-center">
                ...
                <p className="text-sm font-display text-yellow-400 font-bold">
                  {bestDay.dayName}
                </p>
                <p className="text-xs font-mono text-gray-400">
                  {bestDay.xpEarned} XP · {bestDay.questsCompleted} quests
                </p>
              </div>
            )}
            {worstDay && (
              <div className="flex-1 text-center">
                ...
                <p className="text-sm font-display text-red-400 font-bold">
                  {worstDay.dayName}
                </p>
                <p className="text-xs font-mono text-gray-400">
                  {worstDay.xpEarned} XP · {worstDay.questsCompleted} quests
                </p>
              </div>
            )}
            <div className="flex-1 text-center">
              ...
              <p className="text-sm font-display text-purple-400 font-bold">
                {Math.round(thisWeekTotalXp / 7)}
              </p>
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-6">
            {[
              {
                label: "XP",
                thisWeek: thisWeekTotalXp,
                lastWeek: lastWeekTotalXp,
              },
              {
                label: "QUESTS",
                thisWeek: thisWeekTotalQuests,
                lastWeek: lastWeekTotalQuests,
              },
              {
                label: "ACTIVE DAYS",
                thisWeek: thisWeekActiveDays,
                lastWeek: lastWeekActiveDays,
              },
            ].map((item) => {
              const delta = item.thisWeek - item.lastWeek;
              const pct =
                item.lastWeek > 0
                  ? (delta / item.lastWeek) * 100
                  : item.thisWeek > 0
                    ? 100
                    : 0;
              ...
              return (
                <div key={item.label} className="text-center">
                  ...
                  <div className="flex items-center justify-center gap-2">
                    <span className="text-lg font-display font-bold text-gray-500">
                      {item.lastWeek}
                    </span>
                    <span className="text-gray-600">→</span>
                    <span
                      className={`text-lg font-display font-bold ${improved ? "text-green-400" : same ? "text-gray-400" : "text-red-400"}`}
                    >
                      {item.thisWeek}
                    </span>
                  </div>
                  <p
                    className={`text-xs font-mono mt-1 ${improved ? "text-green-400" : same ? "text-gray-500" : "text-red-400"}`}
                  >
                    {same
                      ? "— same"
                      : `${improved ? "↑" : "↓"} ${Math.abs(Math.round(pct))}%`}
                  </p>
                </div>
              );
            })}
        ...
        >
          ...
          <div className="text-right">
            ...
            <p className="text-xs font-display text-purple-400 font-bold">
              FREEBUFF ⚔️
            </p>
          </div>
        </motion.div>
