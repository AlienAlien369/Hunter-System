...
function countDailyQuestsCompletedOnAnyDay(
  dailyQuests: GameState["dailyQuests"],
): number {
  return dailyQuests
...
function lcCompleted(dailyQuests: GameState["dailyQuests"]): number {
  return dailyQuests.filter(
    (q) => q.id.startsWith("LC-") && q.completedDates.length > 0,
  ).length;
}
...
function ssCompleted(dailyQuests: GameState["dailyQuests"]): number {
  return dailyQuests.filter(
    (q) => q.id.startsWith("SS-") && q.completedDates.length > 0,
  ).length;
}
...
function archCompleted(dailyQuests: GameState["dailyQuests"]): number {
  return dailyQuests.filter(
    (q) => q.id.startsWith("AR-") && q.completedDates.length > 0,
  ).length;
}
...
export const ACHIEVEMENTS: AchievementDef[] = [
  ...
  },
  {
    ...
    check: (s) =>
      countDailyQuestsCompletedOnAnyDay(s.dailyQuests) +
        lcCompleted(s.dailyQuests) +
        ssCompleted(s.dailyQuests) +
        archCompleted(s.dailyQuests) >=
      10,
  },
  {
    id: "ACH-003",
  ...
  },
  {
    ...
    check: (s) => {
      const allDates = new Set(
        s.dailyQuests
          .filter((q: GameState["dailyQuests"][number]) =>
            q.id.startsWith("DQ-"),
          )
          .flatMap((q: GameState["dailyQuests"][number]) => q.completedDates),
      );
  ...
  },
  {
    ...
    check: (s) => {
      ...
      return s.dailyQuests.some(
        (q) =>
          (q.id.startsWith("LC-") ||
            q.id.startsWith("SS-") ||
            q.id.startsWith("AR-")) &&
          q.completedDates.length >= 2,
      );
    },
  },
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    check: (s) =>
      s.hiddenQuest?.completedToday === true ||
      s.dailyQuests.some(
        (q: GameState["dailyQuests"][number]) =>
          q.id.startsWith("HQ-") && q.completedDates.length > 0,
      ),
  },
  {
    id: "ACH-602",
    ...
    check: (s) =>
      s.dailyQuests
        .filter((q: GameState["dailyQuests"][number]) => q.id.startsWith("HQ-"))
        .reduce((sum, q) => sum + q.completedDates.length, 0) >= 5,
  },
  ...
  },
  {
    ...
    check: (s) =>
      s.inventory.some((i: GameState["inventory"][number]) =>
        [
          "supreme_rank_stone",
          "title_scroll_legend",
    ...
    check: (s) => {
      ...
      for (const d of allDates) {
        const nutritionDone = s.dailyQuests
          .filter(
            (q: GameState["dailyQuests"][number]) =>
              q.id.startsWith("DQ-") && q.category === "nutrition",
          )
          .filter((q: GameState["dailyQuests"][number]) =>
            q.completedDates.includes(d),
          ).length;
  ...
  },
  {
    ...
    check: (s) => {
      const mma = s.dailyQuests.find(
        (q: GameState["dailyQuests"][number]) => q.id === "DQ-04",
      );
  ...
  {
    id: "ACH-903",
    title: "Sleep disciplined",
    icon: "🌙",
    category: "daily",
    xpReward: 100,
    description: "Sleep by 10:45 PM for 7 days",
    check: (s) => {
      const sleep = s.dailyQuests.find(
        (q: GameState["dailyQuests"][number]) => q.id === "DQ-15",
      );
  ...
  },
  {
    ...
    check: (s) => {
      const saas = s.dailyQuests.find(
        (q: GameState["dailyQuests"][number]) => q.id === "DQ-11",
      );
  ...
  },
  {
    ...
    check: (s) => {
      const sd = s.dailyQuests.find(
        (q: GameState["dailyQuests"][number]) => q.id === "DQ-12",
      );
  ...
  },
  {
    ...
    check: (s) => {
      const sport = s.dailyQuests.find(
        (q: GameState["dailyQuests"][number]) => q.id === "DQ-14",
      );
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    check: (s) =>
      s.profile.rank === "S" &&
      Object.values(s.profile.stats).every((v) => Number(v) >= 30),
  },
  {
    id: "ACH-1102",
    title: "Speedrun",
    ...
    check: (s) => {
      const cats = new Set(
        s.dailyQuests
          ...
          .map((q: GameState["dailyQuests"][number]) => q.category),
      );
