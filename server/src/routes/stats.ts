    ...
    try {
      ...
      switch (period) {
        ...
        default:
          return res
            .status(400)
            .json({
              error: "period must be 'week', 'month', 'quarter' or 'year'",
            });
      }
      ...
      res.json({
        ...
        totals: {
          ...
          completion_rate:
            Math.round((activeDates.size / daysElapsed) * 1000) / 10,
        },
      });
    } catch (error) {
    ...
    try {
      ...
      for (const meta of TRACK_META) {
        ...
        tracks.push({
          ...
          last_completed_at: done.rows[0].last
            ? new Date(done.rows[0].last).toISOString()
            : null,
        });
      }
  ...
  if (missed > 0) {
    ...
    if (prev.rows.length > 0) {
      ...
      penalty = {
        ...
        hp_lost: 0,
        message:
          "You skipped yesterday\u2019s daily quests. Miss today too and the penalty hits: -50 XP, -15 HP.",
      };
    } else {
      ...
      const xpLost = Math.min(
        Math.round(25 * missedDays * (1 - protection)),
        150,
      );
      ...
      const softened =
        protection > 0
          ? ` Your ${brokenStreak}-day streak softened the blow.`
          : "";
      penalty = {
        applied: true,
        missed_days: missedDays,
  ...
  if (currentStreak >= 3) {
    ...
    if (pending.rows.length > 0) {
      const bonusXp = Math.min(
        100,
        Math.max(25, Math.round(0.5 * pending.rows[0].xp_lost)),
      );
      await pool.query(
        "UPDATE users SET xp = xp + $1, updated_at = NOW() WHERE id = $2",
        [bonusXp, userId],
      );
      await pool.query("UPDATE penalties SET recovered = true WHERE id = $1", [
        pending.rows[0].id,
      ]);
      await logActivity(userId, "penalty_recovered", "penalty", {
        bonus_xp: bonusXp,
      ...
      if (last.rows.length > 0) {
        const bonusXp = Math.min(
          100,
          Math.max(25, Math.round(0.5 * last.rows[0].xp_lost)),
        );
        recovery = {
          applied: false,
          bonus_xp: bonusXp,
